...
export class Laser {
    ...
    private distanceBetweenFrames = null;
    ...
    public colision(obj:any, xp:number, yp:number):boolean{
        ...
        for (var i = 0, div = 15, f = false; i < this.point && !r; i+= this.speed/div) {
            ...
            if(obj instanceof Circle || obj instanceof Player){
                ...
                r = false;
                if(!obj.cordOutBounds(x,y)) r = PHY.insidepPoly([x, y], obj.getPoints() );
        ...
        for (var i = 0; i < this.linesRemoving.length; i++) {
                ...
                if(!this.linesRemoving[i].colision){
                    ...
                    if(obj instanceof Circle || obj instanceof Player) this.linesRemoving[i].colision = PHY.getDistanceBetween(this.linesRemoving[i].xD, this.linesRemoving[i].yD, obj.getX(), obj.getY()) < this.distanceBetweenFrames;
                    else if(obj instanceof Polygon){

                        let needCheck = 
                            this.linesRemoving[i].yD > obj.getMinY()-this.distanceBetweenFrames 
                        ...
                        if(needCheck){
                            let point = [this.linesRemoving[i].xD, this.linesRemoving[i].yD];
                            for (let l = 0; !this.linesRemoving[i].colision && l < 5; l+= this.speed/10) {
                                point = [
                    ...
                    else this.linesRemoving[i].colision = this.linesRemoving[i].yD > obj.getMinY()-this.distanceBetweenFrames 
                                                        && this.linesRemoving[i].yD < obj.getMaxY() +this.distanceBetweenFrames
                                                        && this.linesRemoving[i].xD > obj.getMinX() -this.distanceBetweenFrames
                                                        && this.linesRemoving[i].xD < obj.getMaxX()+this.distanceBetweenFrames;    
