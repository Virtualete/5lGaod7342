...
//movidas que estamos en ello y tal

var isOpera = (!!(window as any).opr && !!(window as any).opr.addons) || !!(window as any).opera || navigator.userAgent.indexOf(' OPR/') >= 0;
// Firefox 1.0+
...
function initGameLoop() {
    ...
    if ( (window as any).Worker !== undefined){
        (window as any).ARD = setInterval(function(){
            action();    
            react();
            draw();
        }, 1000/FPS);
...
function action(){
    ...
    elements.forEach(element => {
        if (!(element instanceof Player) ) element.setVisible(screen.objInScreen(element));
    });
}
...
function react(){
    colisionReactElements.forEach(rElement => {
        colisionableElements.forEach(element => {
            if(element.visible() && rElement.colision(element)) rElement.colisionReact(element);
        });
        enemys.forEach(enemy => {
            if(enemy.ply.visible() && rElement.colision(enemy.ply)) rElement.colisionReact(enemy.ply);
        });
    });
    ...
    pj.cleanShadows();
    shadowElements.forEach(sElement => { if(sElement.visible()) pj.updateShadow(sElement) });
...
function draw(){
    ...
    dynamicElements.forEach(element => { if(element.visible()) element.draw(ctx); });
    enemys.forEach(enemy => { if( pj.iSee(enemy.ply)) enemy.ply.draw(ctx);});
    staticElements.forEach(element => { if(element.visible()) element.draw(ctx); });
...
window.onload = function(){
    ...
    setInterval(function(){
        clearInterval((window as any).ARD);
        initGameLoop();
    }, 10000);
    requestRespawn();
