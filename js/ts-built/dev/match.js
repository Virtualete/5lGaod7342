...
var pj = createElement(new Player(UserEvents, Math.floor((Math.random() * 1000)), 10000, '#f4dc42'), 'dynamic', false, true, false);
var screen = new Screen(pj);
...
if (window.Worker) {
    var drawWorker = new Worker('./Threads/draw.js');
}
else {
    window.ARD = setInterval(function () {
        action();
        react();
        draw();
    }, 1000 / FPS);
...
function action() {
    ...
    actionElements.forEach(function (aElement) { aElement.action(); });
    enemys.forEach(function (enemy) { enemy.ply.action(); });
    if (pj.teleporting()) {
        if (!screenTeleport) {
            path = new Path([pj.getX(), pj.getY()], pj, 17);
            UMI.setScreen(new Screen(path));
            screenTeleport = true;
        }
        path.move();
    }
    ...
    elements.forEach(function (element) {
        if (!(element instanceof Player))
            element.setVisible(screen.objInScreen(element));
    });
}
function react() {
    colisionReactElements.forEach(function (rElement) {
        colisionableElements.forEach(function (element) {
            if (element.visible() && rElement.colision(element))
                rElement.colisionReact(element);
        });
        enemys.forEach(function (enemy) {
            if (enemy.ply.visible() && rElement.colision(enemy.ply))
                rElement.colisionReact(enemy.ply);
        });
    });
    enemys.forEach(function (enemy) {
        colisionableElements.forEach(function (element) {
            if (enemy.ply.colision(element))
                enemy.ply.colisionReact(element);
        });
        if (enemy.ply.colision(pj))
            enemy.ply.colisionReact(pj);
    });
    pj.cleanShadows();
    shadowElements.forEach(function (sElement) { if (sElement.visible())
        pj.updateShadow(sElement); });
    enemys.forEach(function (enemy) { pj.updateShadow(enemy.ply); });
    if (pj.getY() < 9000 && (pj.getX() < -2500 || pj.getX() > 3000 || pj.getY() > 1800 || pj.getY() < -1800 && pj.getY() < 9000))
        pj.damage();
}
function draw() {
    bg.draw(ctxBG);
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    dynamicElements.forEach(function (element) { if (element.visible())
        element.draw(ctx); });
    enemys.forEach(function (enemy) { if (pj.iSee(enemy.ply))
        enemy.ply.draw(ctx); });
    staticElements.forEach(function (element) { if (element.visible())
        element.draw(ctx); });
    ...
    enemys.forEach(function (enemy) {
        if (pj.sonarActive())
            enemy.ply.drawBySonar(ctx, pj.getSonarOpacity());
        if (enemy.ply.sonarActive())
            enemy.ply.drawBySonar(ctx, enemy.ply.getSonarOpacity());
    });
...
function createElement(element, typeOfDraw, colisionable, colisionReact, hasShadow) {
    elements.push(element);
    if (typeOfDraw.toLowerCase() == 'dynamic') {
        dynamicElements.push(element);
        actionElements.push(element);
    }
    else if (typeOfDraw.toLowerCase() == 'static')
        staticElements.push(element);
    if (colisionable)
        colisionableElements.push(element);
    if (colisionReact)
        colisionReactElements.push(element);
    if (hasShadow)
