    ...
    Laser.prototype.action = function (controllerData) {
        ...
        for (var i = 0; i < this.linesRemoving.length; i++) {
            if (this.linesRemoving[i].status < 500) {
                ...
                if (PHY.getDistanceBetween(this.linesRemoving[i].x, this.linesRemoving[i].y, this.linesRemoving[i].xD, this.linesRemoving[i].yD) < 10) {
    ...
    Laser.prototype.colision = function (obj, xp, yp) {
        ...
        for (var i = 0; i < this.linesRemoving.length; i++) {
            if (!this.linesRemoving[i].colision) {
                if (obj instanceof Circle || obj instanceof Player) {
                    if (obj.getRadio() > 80) {
                        this.linesRemoving[i].colision = PHY.getDistanceBetween(this.linesRemoving[i].xD, this.linesRemoving[i].yD, obj.getX(), obj.getY()) < obj.getRadio();
                    }
                    else {
                        this.linesRemoving[i].colision = PHY.getDistanceBetween(this.linesRemoving[i].xD, this.linesRemoving[i].yD, obj.getX(), obj.getY()) < 80;
                    }
                }
                else if (obj instanceof Polygon)
                    this.linesRemoving[i].colision = !obj.cordOutBounds(this.linesRemoving[i].xD, this.linesRemoving[i].yD) && PHY.insidepPoly([this.linesRemoving[i].xD, this.linesRemoving[i].yD], obj.getPoints());
                else
                    this.linesRemoving[i].colision = this.linesRemoving[i].yD > obj.getMinY() && this.linesRemoving[i].yD < obj.getMaxY() && this.linesRemoving[i].xD > obj.getMinX() && this.linesRemoving[i].xD < obj.getMaxX();
            }
        }
        if (r) {
